...
FileShare.None);
        ...
        public static void DecompressOneFile(string lpZipFile, string lpTempFile, GZipResult result)
        {
            FileStream fsIn = null;
            GZipStream gzip = null;
            FileStream fsOut = null;
...
FileShare.None);
        ...
        private static void CombineFiles(FileInfo[] files, string lpBaseFolder, string lpTempFile, GZipResult result)
        {
            byte[] buffer;
            int count = 0;
            string fileModDate = null;
        ...
        //将所有文件保存到一个文件，并压缩
        public static GZipResult CompresToOneFile(FileInfo[] files, string lpBaseFolder, string lpDestFolder, string zipFileName, bool deleteTempFile)
        {
            GZipResult result = new GZipResult();
            try
            {
                ...
                //string lpZipFile = lpDestFolder + zipFileName;
                string lpZipFile = Path.Combine(lpDestFolder, zipFileName);

                result.TempFile = lpTempFile;
                result.ZipFile = lpZipFile;
                if (files != null && files.Length > 0)
                {
                    CombineFiles(files, lpBaseFolder, lpTempFile, result);
                    if (result.FileCount > 0)
                    {
                        CreateZipFile(lpTempFile, lpZipFile, result);
                    }
